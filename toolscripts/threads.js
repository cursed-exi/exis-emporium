...
function render() {
    ...
    // The first paragraph is the strand title; any following content belongs
    // in the card body below the details.
    const titleMatch = (strand.content || "").match(/^\s*<p>([\s\S]*?)<\/p>/);
    const strandTitle = titleMatch ? titleMatch[1] : (strand.content || "");
    const strandDetails = titleMatch
        ? strand.content.slice(titleMatch[0].length)
        : "";
